...
def checkFileExists(checkFile):
    if os.path.exists(checkFile):
        ...
        print(f"The {checkFile} file does not exist")
    ...
    with open(fChangeLogs,"w+") as nf:
        ...
        for key in sorted_by_ver.keys():
            try:
                ...
                print(f"Error with: {key}Missing from guids.json")
            ...
            if (cat=="['Toolkit/Publication: Data Parallel C++']"):
                name = f"Pub: Data Parallel C++:](https://www.apress.com/9781484255735)<br>[{name}"

                description=description.replace('*','')
                description=description.replace('fig_','<br>- Fig_')
                description = f"Collection of Code samples for the chapter{description}"

            nf.write(f"|{ver}|[{name}]({url})|{description}" + "|\n")
        nf.write(f"Total Samples: {str(count)}" + "\n\n")
...
def createCodeSamplesList():
    ...
    with open(fCodeSamplesLists,"w+") as nf:
        ...
        for key in sorted_by_name.keys():
            ...
            nf.write(f"|[{name}]({url})|{target}|{description}" + "|\n") 
